...
from liflow.datasets.dataloader.SemanticKITTITemporalAggr import TemporalKITTISet
import warnings
...
class SparseSegmentCollation:
    ...
    def __call__(self, data):
        ...
        return {'pcd_full': torch.stack(batch[0]).float(),
            'pcd_noise': torch.stack(batch[1]).float(),
            'filename': batch[2],
        }

class SemanticKittiDataModule(LightningDataModule):
    def __init__(self, data_dir, seqs, scan_window, num_points, max_range, batch_size, num_workers):
        super().__init__()
    ...
    def val_dataloader(self, pre_training=True):
        ...
        loader = DataLoader(data_set, self.cfg['train']['batch_size'],
                            num_workers=self.num_workers, collate_fn=self.collate)
        return loader
