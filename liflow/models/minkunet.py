...
class MinkIN(nn.Module):
    r"""An instance normalization layer for a sparse tensor."""
    ...
    def forward(self, x):
        ...
        for idx in range(bs):
            mask = (coords[:, 0] == idx)
            curr_tensor = feat[mask].reshape(1, -1, self.num_features).transpose(1, 2).contiguous()  # [B, C, N]
            global_tensor.append(self.instance_norm(curr_tensor)[0].transpose(0, 1).contiguous())  # [ N, C]
...
class MinkGlobalEncIN(nn.Module):
    def __init__(self, **kwargs):
        ...
        in_channels = kwargs.get('in_channels', 3)  # here
...
class MinkUNetDiffIN(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.up1 = nn.ModuleList([
            ...
            nn.Sequential(
                ResidualBlockIN(cs[5] + cs[3], cs[5], ks=3, stride=1,
                                dilation=1, D=self.D),
        ...
        self.up2 = nn.ModuleList([
            ...
            nn.Sequential(
                ResidualBlockIN(cs[6] + cs[2], cs[6], ks=3, stride=1,
                                dilation=1, D=self.D),
        ...
        self.up3 = nn.ModuleList([
            ...
            nn.Sequential(
                ResidualBlockIN(cs[7] + cs[1], cs[7], ks=3, stride=1,
                                dilation=1, D=self.D),
        ...
        self.up4 = nn.ModuleList([
            ...
            nn.Sequential(
                ResidualBlockIN(cs[8] + cs[0], cs[8], ks=3, stride=1,
                                dilation=1, D=self.D),
        ...
        self.last = nn.Sequential(
            nn.Linear(cs[8], 20),  # 96 -> 96 or 96 -> 32 ...
            nn.LeakyReLU(0.1, inplace=True),
            nn.Linear(20, 3),
        )
